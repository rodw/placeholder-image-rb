...
**Bounded work per request.** `image_max_dim_px` and `image_max_total_px` cap the CPU time and memory a single request can consume. Under [the default configuration](#configuration) in the worst case a transient ~48 MB buffer is created, consuming ~0.10 seconds of CPU.
...
Strict pattern matching is applied to all inputs. No client-supplied text is ever rendered into images.
